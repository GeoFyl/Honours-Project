    ...
    namespace PerRangeReport {
        ...
        inline void UpdateProfilerResults(NVPW_MetricsEvaluator* pMetricsEvaluator, const ReportLayout& reportLayout, const ReportData& reportData)
        {
            ...
            for (const ReportData::RangeData& rangeData : reportData.ranges)
            {
                ...
                if (range_test_results.size() < rangeData.perRangeReportValues.size()) {
                    range_test_results.resize(rangeData.perRangeReportValues.size());
                }

                for (int i = 0; i < rangeData.perRangeReportValues.size(); i++)
                {
...
namespace nv { namespace perf { namespace profiler {
    ...
    public:
            ...
            const std::vector<std::string>& additionalMetrics)
        {
            ...
            m_reportLayout.perRange.definition = Profiler::GetCustomReportDefinition();
                ...
                if (decodeResult.allStatisticalSamplesCollected)
                {
                    [&]() {
                        ...
                        if (outputOptions.enableCsvReport)
                        {
                            ...
                            if (ProfilerGlobal::remaining_captures_ != Profiler::GetTotalCaptures()) {
                                PerRangeReport::UpdateProfilerResults(m_metricsEvaluator, m_reportLayout, reportData);
                            }
