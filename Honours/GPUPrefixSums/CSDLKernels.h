...
namespace CSDLKernels {
    ...
    class InitCSDL : ComputeKernelBase {
       public:
        InitCSDL(ID3D12Device* device,
                 const std::vector<std::wstring>& compileArguments,
                 const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, shaderPath, L"InitChainedScan", compileArguments,
    ...
    class CSDLExclusive : ComputeKernelBase {
       public:
        CSDLExclusive(ID3D12Device* device,
                      const std::vector<std::wstring>& compileArguments,
                      const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, shaderPath, L"ChainedScanDecoupledLookbackExclusive",
