...
class ComputeKernelBase {
   ...
   public:
    ComputeKernelBase(ID3D12Device* device,
        ...
        auto byteCode = CompileShader(shaderPath, entryPoint, compileArguments);
   ...
   private:
    std::vector<uint8_t> CompileShader(const std::filesystem::path& shaderPath,
        ...
        winrt::check_hresult(utils->BuildArguments(
            shaderPath.wstring().c_str(), entryPoint, L"cs_6_0",
    ...
    void CreatePipelineStateDesc(ID3D12Device* device,
